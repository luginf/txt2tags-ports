...
from . import state
from .utils import Debug, getUnknownErrorMessage
from .cli import CommandLine
from .config import ConfigMaster
from .converter import convert_this_files, process_source_file
from .output import finish_him


def load_GUI_resources():
    "Load all extra modules and methods used by state.GUI"
    # global (use state.) Tkinter, tkFileDialog, tkMessageBox
...
class Gui:
    ...
    def askfile(self):
        ...
        newfile = tkinter.filedialog.askopenfilename(filetypes=ftypes)
        if newfile:
    ...
    def runprogram(self):
        # global (use state.) CMDLINE_RAW
        # Prepare
        self.target_name2key()
        infile, target = self.infile.get(), self.target.get()
        # Sanity
        if not target:
            tkinter.messagebox.showwarning(my_name, _("You must select a target type!"))
            return
        if not infile:
        ...
        # Compose cmdline
        guiflags = []
        real_cmdline_conf = ConfigMaster(state.CMDLINE_RAW).parse()
        ...
        for opt in self.checks:
            ...
            if val != on_config or (
                ...
                if val:
                    # Was not set, but user selected on state.GUI
                    ...
                    # Was set, but user deselected on state.GUI
        ...
        # Run!
        cmdline_raw_orig = state.CMDLINE_RAW
        try:
            # Fake the state.GUI cmdline as the real one, and parse file
            state.CMDLINE_RAW = CommandLine().get_raw_config(cmdline[1:])
            data = process_source_file(infile)
            # On state.GUI, convert_* returns the data, not finish_him()
            outlist, config = convert_this_files([data])
            # On state.GUI and STDOUT, finish_him() returns the data
            ...
            # Show the "file saved" message
                ...
                tkinter.messagebox.showinfo(my_name, msg)
        except error:  # common error (windowed), not quit
            pass
        except:  # fatal error (windowed and printed)
            errormsg = getUnknownErrorMessage()
            print(errormsg)
            tkinter.messagebox.showerror(_("%s FATAL ERROR!") % my_name, errormsg)
            self.exit()
        state.CMDLINE_RAW = cmdline_raw_orig
    ...
    def mainwindow(self):
        ...
        if outfile == STDOUT:  # map -o-
            self.conf["stdout"] = 1
        if self.conf.get("headers") is None:
